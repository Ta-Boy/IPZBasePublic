...
// MARK: - 常用属性
    ...
    // FIXME: 不使用分辨率来适配, 改为使用 safeAreaInsets
    /*
    public static let iPhoneX: Bool = ((UIScreen.instancesRespond(to: #selector(getter: UIScreen.currentMode)) ? __CGSizeEqualToSize(CGSize(width: 1125.0, height: 2436.0), (UIScreen.main.currentMode?.size)!) : false) || (UIScreen.instancesRespond(to: #selector(getter: UIScreen.currentMode)) ? __CGSizeEqualToSize(CGSize(width: 1242.0, height: 2688.0), (UIScreen.main.currentMode?.size)!) : false)
            || (UIScreen.instancesRespond(to: #selector(getter: UIScreen.currentMode)) ? __CGSizeEqualToSize(CGSize(width: 828.0, height: 1792.0), (UIScreen.main.currentMode?.size)!) : false))
     */
...
public extension GlobalProperties {
    ...
    /// 顶部安全区域的高度
    /// @note: 仅针对竖屏模式
    public static var kTopSafeHeight: CGFloat {
        return iPhoneX ? 44 : 0
    }

    /// 底部的安全区域
    /// @note: 仅针对竖屏模式
    public static var kBottomSafeHeight: CGFloat {
